...
/* Whether or not we are a remote Angband, slaved to some parent */
static BOOL running_as_remote;
static NSConnection *remote_connection;
static size_t remote_shmem_size;
static void *remote_shmem_buffer;
...
@public
    ...
    /* The buffered image */
    BOOL bufferWithLayer;
    CGLayerRef angbandLayer;
    CGContextRef angbandBitmap;
...
{
    return inLiveResize && graf_mode != GRAF_MODE_NONE && ! running_as_remote;
}
...
- (NSSize)scaleFactor
{
    ...
    if (bufferWithLayer)
    {
        currentSize = CGLayerGetSize(angbandLayer);
    }
    else
    {
        currentSize.width = CGBitmapContextGetWidth(angbandBitmap);
        currentSize.height = CGBitmapContextGetHeight(angbandBitmap);
    }
...
- (void)updateImage
{
    ...
    if (activeView)
    {
        ...
        {
            size = [activeView angbandViewportSize];
        }
    }
    ...
    {
        size_t bitmapWidth = MIN(size.width, 1920);
        size_t bitmapHeight = MIN(size.height, 1080);

        if (angbandBitmap) CFRelease(angbandBitmap);
        CGColorSpaceRef space = CGColorSpaceCreateDeviceRGB();
        angbandBitmap = CGBitmapContextCreate(remote_shmem_buffer /* may be null! */, bitmapWidth, bitmapHeight, 8, bitmapWidth * 4, space, kCGImageAlphaPremultipliedFirst | kCGBitmapByteOrder32Host);
...
- (void)viewWillStartLiveResize:(AngbandView *)view
{
    ...
    if (inLiveResize == 1 && graf_mode != GRAF_MODE_NONE)
    {
        ...
        [self setNeedsDisplay]; //we'll need to redisplay everything anyways, so avoid creating all those little redisplay rects
        [self requestRedraw];
    }
#endif
}
...
- (void)drawRect:(NSRect)rect inView:(NSView *)view
{
    ...
    {
        NSRect bounds = [view bounds];
        CGImageRef image = CGBitmapContextCreateImage(angbandBitmap);
        CGContextSetBlendMode(context, kCGBlendModeCopy);
        CGContextDrawImage(context, *(CGRect *)&bounds, image);
...
- (CGContextRef)lockFocusUnscaled
{
    ...
    CGContextRef ctx = (bufferWithLayer ? CGLayerGetContext(angbandLayer) : angbandBitmap);
...
- (NSSize)imageSize
{
    ...
    {
        return NSMakeSize(CGBitmapContextGetWidth(angbandBitmap), CGBitmapContextGetHeight(angbandBitmap));
    }
}
...
- (id)init
{
    ...
    {        
        ...
        /* If we're not remote, we use a layer; otherwise we use a bitmap context */
#if ALWAYS_BUFFER_WITH_BITMAP
        bufferWithLayer = 0;
#else
        bufferWithLayer = ! running_as_remote;
#endif
...
- (NSWindow *)makePrimaryWindow
{
    if (! primaryWindow && ! running_as_remote)
    {
...
- (NSRect)convertBaseRect:(NSRect)rect toView:(id)angbandView
{
    if (! angbandView) return NSZeroRect;
    
    NSSize ourSize = [self baseSize];
    NSSize theirSize = [angbandView angbandViewportSize];
...
- (void)setNeedsDisplay
{
    ...
    {
        [angbandView setNeedsDisplayAtNextRefresh];
    }
}
...
static void Term_init_cocoa(term *t)
{
    ...
    if (! running_as_remote) NSDisableScreenUpdates();
    ...
    if (running_as_remote)
    {
        id remoteView = (AngbandView *)[remote_connection rootProxy];
        [context addAngbandView:remoteView];
        [remoteView setAngbandContext:context];
    }
    ...
    if (! running_as_remote) NSEnableScreenUpdates();
...
static errr Term_xtra_cocoa(int n, int v)
{
    ...
    switch (n)
    {
        ...
        {        
            ...
            [angbandContext setNeedsDisplay];
            /* Success */
            break;
        }
...
static void load_sounds(void)
{
    /* Don't do this if we're remote */
    if (running_as_remote) return;
