...
{
    /* Calculate the age */
    p_ptr->age = rp_ptr->b_age + randint0(rp_ptr->m_age);
...
void resurrect_borg(void)
{
	...
	/* borg_clear_3(); until resurrect crash is fixed */
	/* borg_init_3(); until resurrect crash is fixed */
...
void do_cmd_borg(void)
{
    ...
    /* Command: check previous steps */
        ...
        case 'K':
        {
            ...
            /* Get keypress */
            msg_format("There are %d missing monsters.", n);
            msg_print(NULL);
