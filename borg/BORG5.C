...
static void borg_update_kill_new(int i)
{
	 ...
	 */
	kill->m_idx = cave_m_idx[kill->y][kill->x];
...
static bool observe_kill_move(int y, int x, int d, byte a, char c, bool flag)
{
    ...
    for (i = 1; i < borg_kills_nxt; i++)
    {
		...
		if (kill->m_idx != cave_m_idx[y][x]) continue;
        ...
        if (z)
        {
            ...
            /* Note */
            if (borg_verbose) borg_note(format("# Tracking a monster '%s' at (%d,%d) from (%d,%d)",
                             (r_ptr->name),
                             kill->y, kill->x, ox, oy));
