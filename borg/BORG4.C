        ...
        switch (item->tval)
        {
            ...
            switch (item->sval)
            {
                ...
                borg_skill[BI_ATELEPORTLVL] += item->iqty;
               	if (!borg_munchkin_start || borg_skill[BI_MAXCLEVEL] > borg_munchkin_level) borg_skill[BI_ATELEPORT] += 1;
                break;
...
static s32b borg_power_aux1(void)
{
	 ...
	 */
	if (borg_class == CLASS_MAGE) value = value * 3 / 10;
    ...
    if (borg_skill[BI_CRSAGRV] && borg_skill[BI_CDEPTH] == 100 && borg_skill[BI_KING] <= 0) value -= 1000L;
    ...
    if (cp_ptr->spell_book && ((cur_wgt - max_wgt) / 10) > 0)
    {
...
static s32b borg_power_aux2(void)
{
    ...
    /* Reward fuel */
    k = 0;
    for (; k < 7 && k < borg_skill[BI_AFUEL]; k++) value += 60000L;
    ...
    k = 0;
	if (borg_skill[BI_CLEVEL] >=3 && borg_has[lookup_kind(TV_SCROLL, SV_SCROLL_TELEPORT_LEVEL)] == 0)
	{
		if (borg_skill[BI_ATELEPORT] ) value += 10000L;
	}
	if (borg_skill[BI_CLEVEL] >= 7 && borg_has[lookup_kind(TV_SCROLL, SV_SCROLL_TELEPORT_LEVEL)] == 0)
	{
    ...
    if (borg_skill[BI_MAXDEPTH] >= 99)
    {
        ...
        if (borg_race_death[546] != 0)
        {
            ...
            else
            {
                ...
                for (; k < 99 && k < borg_skill[BI_AEZHEAL]; k++) value +=10000L;
				k = 0;
				for (; k < 99 && k < borg_skill[BI_ALIFE]; k++) value +=20000L;
                k = 0;
                for (; k < 99 && k < borg_skill[BI_ASPEED]; k++) value +=8000L;
                k = 0;
				if (borg_class != CLASS_WARRIOR)
	...
	else
	{
		...
		if (amt_enchant_to_a  && my_need_enchant_to_a) value += 540L;

		/* Reward enchant weapon to hit */
		if (amt_enchant_to_h && my_need_enchant_to_h) value += 540L;

		/* Reward enchant weapon to damage */
		if (amt_enchant_to_d && my_need_enchant_to_d) value += 500L;

		/* Reward *enchant weapon* to damage */
		if (amt_enchant_weapon && borg_skill[BI_MAXDEPTH] < 70) value += 5000L;

		/* Reward *enchant armour*  */
		if (amt_enchant_armor && borg_skill[BI_MAXDEPTH] < 70) value += 5000L;
	}
...
static s32b borg_power_home_aux2(void)
{
    ...
    /* Collect ident */
    for (k = 0; k < 50 && k < num_ident; k++) value += 500L - k*10L;
    ...
    /* Collect teleport */
    for (k = 0; k < 85 && k < num_teleport; k++) value += 8000L;
...
#if 0
	for (k = 0; k < 90 && k < num_fix_stat[6]; k++) value += 1000L;
    for (k = 0; k < 90 && k < num_mush_second_sight; k++) value += 1000L;
    for (k = 0; k < 90 && k < num_mush_fast_recovery; k++) value += 1000L;
    for (k = 0; k < 90 && k < num_mush_cure_mind; k++) value += 1000L;
    for (k = 0; k < 90 && k < num_mush_emergency; k++) value += 1000L;
    /* for (k = 0; k < 90 && k < num_mush_terror; k++) value += 1000L; */
    for (k = 0; k < 90 && k < num_mush_stoneskin; k++) value += 1000L;
    for (k = 0; k < 90 && k < num_mush_debility; k++) value += 1000L;
    for (k = 0; k < 90 && k < num_mush_sprinting; k++) value += 1000L;
    /* for (k = 0; k < 90 && k < num_mush_purging; k++) value += 1000L; */
#endif 
...
int borg_danger(int y, int x, int c, bool average, bool full_damage)
{
	...
	if (!(cave_info[y][x] & (CAVE_ICKY)) && borg_skill[BI_CDEPTH] <= 80)
	{
...
/*** Essential Items for Level 46 to 55 ***/
    ...
    if (borg_stat[A_STR] < 18+20) return ("low STR");

    if (cp_ptr->spell_book == TV_MAGIC_BOOK)
    {
        if (borg_stat[A_INT] < 18+40) return ("low INT");
    }
    if (cp_ptr->spell_book == TV_PRAYER_BOOK)
    {
        if (borg_stat[A_WIS] < 18+40) return ("low WIS");
    }
    if (borg_stat[A_DEX] < 18+40) return ("low DEX");
    if (borg_stat[A_CON] < 18+40) return ("low CON");
